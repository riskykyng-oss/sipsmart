...
const steps = [
  ...
  },
  {
    ...
    icon: CreditCard,
    title: "Order & Pay",
    description: "Secure checkout with EcoCash, InnBucks, Visa or cash on delivery.",
  },
  {
    number: "04",
    icon: Truck,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step) => (
            <div key={step.number} className="text-center relative">
              <div className="w-14 h-14 bg-green-900 rounded-2xl flex items-center justify-center mx-auto mb-5 relative">
                <step.icon className="size-6 text-gold-500" />
                <span className="absolute -top-2 -right-2 w-6 h-6 bg-gold-500 text-green-900 text-xs font-bold rounded-full flex items-center justify-center">
                  {step.number}
                </span>
              </div>
